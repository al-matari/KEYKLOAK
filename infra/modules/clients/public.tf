resource "keycloak_openid_client" "public" {
  for_each = local.public_clients

  realm_id  = var.realm_id
  client_id = each.value.client_id

  name        = try(each.value.name, null)
  description = try(each.value.description, null)
  enabled     = try(each.value.enabled, null)

  access_type = "PUBLIC"

  root_url                        = each.value.root_url
  base_url                        = try(each.value.base_url, null)
  admin_url                       = try(each.value.admin_url, null)
  valid_redirect_uris             = each.value.valid_redirect_uris
  valid_post_logout_redirect_uris = try(each.value.valid_post_logout_redirect_uris, null)
  web_origins                     = each.value.web_origins

  standard_flow_enabled        = true
  implicit_flow_enabled        = false
  direct_access_grants_enabled = false
  service_accounts_enabled     = false
  consent_required             = try(each.value.consent_required, null)
  full_scope_allowed           = try(each.value.full_scope_allowed, null)
  pkce_code_challenge_method   = try(each.value.pkce_code_challenge_method, null)
  require_dpop_bound_tokens    = try(each.value.require_dpop_bound_tokens, null)
  extra_config                 = try(each.value.extra_config, null)
}

resource "keycloak_openid_client_default_scopes" "public" {
  for_each = local.public_clients_with_default_scopes

  realm_id       = var.realm_id
  client_id      = keycloak_openid_client.public[each.key].id
  default_scopes = each.value.default_scopes
}

resource "keycloak_openid_client_optional_scopes" "public" {
  for_each = local.public_clients_with_optional_scopes

  realm_id        = var.realm_id
  client_id       = keycloak_openid_client.public[each.key].id
  optional_scopes = each.value.optional_scopes
}
