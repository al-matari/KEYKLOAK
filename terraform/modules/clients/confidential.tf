resource "keycloak_openid_client" "confidential" {
  for_each = local.confidential_clients
  ...
  valid_post_logout_redirect_uris = try(each.value.valid_post_logout_redirect_uris, null)
  ...
  consent_required                = try(each.value.consent_required, null)
  full_scope_allowed              = try(each.value.full_scope_allowed, null)
  use_refresh_tokens              = try(each.value.use_refresh_tokens, null)
...
resource "keycloak_openid_client_default_scopes" "confidential" {
  for_each = local.confidential_clients_with_default_scopes

  realm_id       = var.realm_id
  client_id      = keycloak_openid_client.confidential[each.key].id
  default_scopes = each.value.default_scopes
}

resource "keycloak_openid_client_optional_scopes" "confidential" {
  for_each = local.confidential_clients_with_optional_scopes
